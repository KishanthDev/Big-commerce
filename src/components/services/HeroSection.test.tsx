...
describe("HeroSection Component", () => {
  ...
  test("renders with default values when no props provided", () => {
    ...
    expect(
      screen.getByText("Welcome to Our Auto Repair Shop"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/We provide top-notch auto repair services/),
    ).toBeInTheDocument();
  });
  ...
  test("renders both buttons", () => {
    ...
    expect(scheduleButton).toHaveClass(
      "bg-red-600",
      "rounded-2xl",
      "text-white",
    );
  ...
  test("content container has correct styling", () => {
    ...
    expect(container).toHaveClass("relative", "z-10", "max-w-4xl", "px-4");
  });
