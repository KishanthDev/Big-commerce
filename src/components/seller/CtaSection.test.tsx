...
describe('CtaSection Component', () => {
  ...
  test('displays the main heading', () => {
    render(<CtaSection />);
    expect(
      screen.getByRole('heading', { name: /Ready to Discover Amazing Products?/i })
    ).toBeInTheDocument();
  });
  ...
  test('shows the subheading text', () => {
    render(<CtaSection />);
    expect(
      screen.getByText(/Join thousands of sellers who are thriving on our platform/i)
    ).toBeInTheDocument();
  });

  test('renders both action buttons', () => {
    render(<CtaSection />);
    expect(screen.getByRole('button', { name: /Start Selling Today/i })).toBeInTheDocument();
  ...
  test('primary button has correct styling', () => {
    render(<CtaSection />);
    const primaryButton = screen.getByRole('button', { name: /Start Selling Today/i });
