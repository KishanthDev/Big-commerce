...
describe('Footer Component', () => {
  test('renders copyright text', () => {
    render(<Footer />);
    expect(screen.getByText(/© 2025 Mike's Auto Service - All Rights Reserved/i)).toBeInTheDocument();
  });
  ...
  test('applies correct text styling', () => {
    ...
    expect(paymentText).toHaveClass('text-sm', 'md:text-base', 'mt-2', 'md:mt-3');
  });
  ...
  test('applies correct dark mode classes', () => {
    ...
    const paymentIcons = screen.getAllByTestId('payment-icon');
    expect(paymentIcons[0]).toHaveClass('dark:bg-gray-200', 'dark:text-gray-900');
  });
  ...
  test('payment icons container has correct layout', () => {
    ...
    expect(iconsContainer).toHaveClass(
      'flex', 'justify-center', 'gap-4', 'mt-2', 'md:mt-3'
    );
  });
});