...
describe('AdditionalInfoSection', () => {
  it('renders the section with heading and information', () => {
    ...
    // Check heading
    const heading = screen.getByRole('heading', { name: /Additional Information/i });
    expect(heading).toBeInTheDocument();

    expect(screen.getByText(/Makes Serviced:/i)).toBeInTheDocument();
    expect(screen.getByText(/All domestic and foreign vehicles/i)).toBeInTheDocument();

    expect(screen.getByText(/Warranty:/i)).toBeInTheDocument();
    expect(screen.getByText(/24-month \/ 24,000 mile warranty on parts and labor/i)).toBeInTheDocument();
    ...
    expect(screen.getByText(/Available for major repairs \(reservation required\)/i)).toBeInTheDocument();
