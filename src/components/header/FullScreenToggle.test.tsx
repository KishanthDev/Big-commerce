...
beforeAll(() => {
  ...
  jest.spyOn(document, 'addEventListener').mockImplementation(
    mockFullscreenAPI.addEventListener
  );
  jest.spyOn(document, 'removeEventListener').mockImplementation(
    mockFullscreenAPI.removeEventListener
  );
});
...
describe('FullScreenToggle', () => {
  ...
  it('updates state when fullscreen changes', () => {
    ...
    mockFullscreenAPI._fullscreenElement = document.documentElement;
    const changeHandler = mockFullscreenAPI.addEventListener.mock.calls
      .find(([type]) => type === 'fullscreenchange')?.[1];
